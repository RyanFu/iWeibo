...
class Model_Recommend extends Core_Model
{
     ...
     * @param int $lastid    翻页界限 (推荐用户本地id)
     */
    public function getHotUserList($reqnum, $pageflag, $lastid)
    {
        if(!empty($pageflag) && !empty($lastid))
        {
            if($pageflag == 1)
            {
                $sql= 'SELECT * FROM `'.$this->_tableName.'` WHERE `id`<'.$lastid.' ORDER BY `id` DESC LIMIT '.$reqnum;
            }else{
                $sql= 'SELECT * FROM `'.$this->_tableName.'` WHERE `id`>'.$lastid.' ORDER BY `id` LIMIT '.$reqnum;
            }
        }else{
            $sql= 'SELECT * FROM `'.$this->_tableName.'` ORDER BY `id` DESC LIMIT '.$reqnum;
        }
        $userList = $this->getAll($sql, 0, null, 'account');
        /* 保证数组按id逆序 */
        if($pageflag == 2)
        {
    ...
    /**
     * 根据推荐用户本地id列表获取对应的用户信息
     * @param array $arrID    推荐用户本地id列表
     */
    public function getHotUserByIDs($arrID)
    {
        $sql = 'SELECT * FROM `'.$this->_tableName.'` WHERE `id` IN ('.implode(',', $arrID).')';
        return $this->getAll($sql, 0, null, 'account');
    }
    
    /**
     * 获取推荐用户总数
     */
    public function getHotUsersCount()
    {
        $sql = 'SELECT COUNT(*) AS C FROM `'.$this->_tableName;
        $c = $this->fetchOne($sql);
        return intval($c['C']);
    }
}
