...
class Model_Hottopic extends Core_Model
{
     ...
     * @param int $lastid    翻页界限 (话题本地id)
     */
    public function getHotTopicList($reqnum, $pageflag, $lastid)
    {
        if(!empty($pageflag) && !empty($lastid))
        {
            if($pageflag == 1)
            {
                $sql= 'SELECT * FROM `'.$this->getTableName().'` WHERE `id`<'.$lastid.' ORDER BY `id` DESC LIMIT '.$reqnum;
            }else{
                $sql= 'SELECT * FROM `'.$this->getTableName().'` WHERE `id`>'.$lastid.' ORDER BY `id` LIMIT '.$reqnum;
            }
        }else{
            $sql= 'SELECT * FROM `'.$this->getTableName().'` ORDER BY `id` DESC LIMIT '.$reqnum;
        }
        $topicList = $this->getAll($sql, null, 0 , 'name');
        /* 保证数组按id逆序 */
        if($pageflag == 2)
        {
            $topicList = array_reverse($topicList);
        }
        return $topicList;
    }
     ...
     * 获取设置的热门话题总数
     */
    public function getHotTopicsCount()
    {
