...
class Model_Blog extends Core_Model
{
    ...
    public static function addBlog ($blog)
    {
        ...
        if (!empty ($_topics)) {
            ...
            {
                Model_Topic::addTopic ($topic, true,  $_blog);
            }
        }
        return $r;
    }
...
INNER JOIN `##__mb_topic` AS topic ON r.topicid = topic.tid ' . $where;
    ...
    public function unLockWallBlog ($opentids)
    {
        ...
        $sql = 'UPDATE `' . $this->_tableName . '` SET `censortime` =' . Core_Fun::time () . '  WHERE `opentid` IN (' . $_opentidstr . ')  ';
        $this->query ($sql);
        $sql = 'UPDATE `##__mb_topicblog` SET `visible` = 1 , `censortime` =' . Core_Fun::time () . ' 
        	    WHERE `opentid` IN (' . $_opentidstr . ') and `visible` = 0 ';
        $this->query ($sql);
		return Core_Db::affected_rows();
    }
    ...
    /**
     * 获取本地微博的(id, opentid)对 
     * @param int $reqnum    请求数目
     * @param int $pageflag  翻页标识  第一页:0, 向下翻页:1, 向上翻页:2
     * @param int $lastid    翻页界限 (本地微博id)
     * @param int $type      微博类型  0x1:原创;0x2:转播;0x8:对话;0x10:空回;0x20:提及;0x40:评论   
     *                               多个类型，使用或操作
     */
    public function getLatestBlogIDs($reqnum, $pageflag, $lastid, $type)
    {
        $obj = new self();
        $conditions = ' `visible`=1';
        if(!empty($pageflag) && !empty($lastid))
        {
            if($pageflag == 1)
            {
                $conditions .= ' AND `id`<'.$lastid;
            }else{
                $conditions .= ' AND `id`>'.$lastid;
            }
            if($pageflag == 1)
            {
                $sql = 'SELECT `opentid`,`id` FROM `'.$this->_tableName.'` WHERE `visible`=1 AND `id`<'.$lastid.' ORDER BY `id` DESC LIMIT '.$reqnum;
            }else{
                $sql = 'SELECT `opentid`,`id` FROM `'.$this->_tableName.'` WHERE `visible`=1 AND `id`>'.$lastid.' ORDER BY `id` LIMIT '.$reqnum;
            }
        }
        if(!empty($type))
        {
            $conditions .= ' AND ((1<<(`type`-1))&'.$type.')>0';
        }
        if($pageflag == 2)
        {
            $sql = 'SELECT  `opentid`, MAX(`id`) AS `id` FROM `'.$this->_tableName.'` WHERE'.$conditions.' GROUP BY `opentid` ORDER BY `id` LIMIT '.$reqnum;
        }else{
            $sql = 'SELECT  `opentid`, MAX(`id`) AS `id` FROM `'.$this->_tableName.'` WHERE'.$conditions.' GROUP BY `opentid` ORDER BY `id` DESC LIMIT '.$reqnum;
        }
        $ids = $obj->getAll($sql, null, 0, 'opentid');
        /* 保证数组按id逆序 */
        if($pageflag == 2)
        {
    ...
    /**
     * 获取可见（未屏蔽）微博的边界ID
     * @param $direction 0:查找最小值；1：查找最大值；
     */
    public function getBoundaryBlogID($direction)
    {
        $obj = new self();
        if($direction == 1)
        {
            $sql = 'SELECT `id` FROM `'.$this->_tableName.'` WHERE `visible`=1 ORDER BY `id` DESC LIMIT 1';
        }else{
            $sql = 'SELECT `id` FROM `'.$this->_tableName.'` WHERE `visible`=1 ORDER BY `id` LIMIT 1';
        }
        $boundary = $obj->fetchOne($sql);
        return $boundary['id'];
    }
}