...
- (void)attachImage:(UIImage *)imageRef {
	...
	//
//	[[baseView layer] setCornerRadius:8.0f];
//	[[baseView layer] setMasksToBounds:YES];
	baseView.image = [UIImage imageNamed:@"composeImageBigFrame.png"];
...
//	[DelBtnBase setImage:[UIImage imageNamed:@"loginBg.png"] forState:UIControlStateNormal];
//	[DelBtnBase setImage:[UIImage imageNamed:@"loginBg.png"] forState:UIControlStateHighlighted];
...
-(void)imageTaped:(UITapGestureRecognizer *)recognizer {
	if ([self.myAttachedPictureCtrlViewDelegate respondsToSelector:@selector(picCtrlViewImageTouched:)]) {
		[self.myAttachedPictureCtrlViewDelegate picCtrlViewImageTouched:recognizer];
	}
}
@end
