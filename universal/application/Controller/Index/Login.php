...
class Controller_Index_Login extends Core_Controller_Action
{
    ...
    {
        $cacheFile = '_data/_msglist/login'.$num.'.php';
        $strongCacheFile = '_data/_msglist/stronglogin'.$num.'.php';
        if(!$msglist = Core_Cache::read($cacheFile))
        {
            $apiClient = Core_Open_Api::getNoneTokenClient();
            $p = array('p' => 0, 'n' => $num);
            if($apiClient->testPublic($p) == 0)
            {
            	$openmsg = $apiClient->getPublic($p);
            	$msglist = $openmsg['data']['info'];
           		$msglist && Core_Lib_Base::formatTArr($msglist);
          	    Core_Cache::write($cacheFile,$msglist, 3);
          	    Core_Cache::write($strongCacheFile, $msglist, 3600*12);
            }else if(!$msglist = Core_Cache::read($strongCacheFile)) {    	
    ...
    public function lAction()
    {
        ...
        try{
            ...
            Model_Stat::addStat('login', $name);
        }catch(Exception $e)
        {}
        $this->showmsg('', 'javascript:top.location.href="' . Core_Fun::getPathroot() . 'u/'.$name.'";', 0);
    }
        ...
        if($this->getParam("oauth_token") && $this->getParam("oauth_verifier") && $this->getParam("oauth_token") == $_SESSION["request_token"])
        {
            ...
            if($cUser['uid'])
            {
                if($user)
                {
                    ...
                    $seogourl = Core_Fun::getPathroot() . 'login';
                    $_SESSION["tmp_accesstoken"] =$oauthToken;
                    $_SESSION["tmp_accesstoken_secret"] = $oauthTokenSecret;
                    //$btntext="绑定腾讯微博";
                    //$this->assign('btntext', $btntext);
                    $this->assign('btns',array(array("text"=>"使用绑定帐号登录","link"=>$directlogin),array("text"=>"绑定腾讯微博","link"=>$seogourl)));
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        $this->showmsg('','login/index/msg/'.Core_Comm_Modret::RET_USER_UNBOUND,0);
                    }
                }
            }
        }
