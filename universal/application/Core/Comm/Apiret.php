...
class Core_Comm_Apiret
{
	...
	const RET_AUTH_APIPARAMERROR = 1001; //api调用参数错误
	
	const RET_AUTH_IPLIMIT = 2001;  //ip受限
	const RET_AUTH_FREQUENCYLIMIT = 2002; //操作频繁，请稍后再试(ret=1,errcode=2,3,4,5,6)
	...
	const RET_AUTH_NO_BIND= 4109; //该用户没有授权绑定		
    
	const RET_SERVER_ERROR = -5;
	const RET_SERVER_DATANOTFULL = 5001;//获取微博异常,data为null
	
	const RET_SERVICE_ERROR = -6;  //服务错误， ret=4的大类错误提示
	const RET_RESPONSE_ERROR = -7;  //API响应异常
	//msg
	public static $ARR_MSG 
		= array(self::RET_SUCC => "成功"
				, self::RET_AUTH_APIPARAMERROR => "api调用参数错误"
				...
				, self::RET_SEND_SENDTOOFAST => "发表过于频繁，请稍后再发"
				, self::RET_SEND_SOURCEDELETED => "源消息已删除"
				, self::RET_SEND_SOURCEREVIEW => "源消息审核中"
				, self::RET_SEND_REPEAT => "请不要连续发送重复的内容"

				, self::RET_SERVER_DATANOTFULL => "api错误,data为空"
				
				, self::RET_SERVER_ERROR => "系统错误"
				, self::RET_SERVICE_ERROR => "服务错误"
				, self::RET_RESPONSE_ERROR => "API响应异常"
				);
