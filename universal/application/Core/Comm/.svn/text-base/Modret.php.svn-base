...
class Core_Comm_Modret
{
    ...
    const RET_MOBILE_SYSTEMERROR = -300;    //（手机客户端）系统错误, 具体原因在msg中反映
    const RET_API_KEYMISS = -301;    //appkey 或者 appsecret丢失
    const RET_MOBILE_CHECKSIGNATUREFAIL = -302;    //(手机客户端）验证签名错误
    const RET_LOCAL_SAVEERROR = -303;    //消息存储失败
    const RET_MOBILE_USERINFOERROR = -304;    //(手机客户端）获取用户信息失败
    const RET_MOBILE_DELFRIENDERROR = -305;    //（手机客户端）取消收听失败
    const RET_MOBILE_ADDFRIENDERROR = -306;    //（手机客户端）收听失败
    const RET_MOBILE_CHECKFRIENDERROR = -307;    //（手机客户端）检测本地收听关系失败
    const RET_NET_CONNECTERROR = -308;    //服务器网络通讯失败
    const RET_LOGINSTATE_INVALID = -309;    //登录态失效
    const RET_LOGINSTATE_FAIL = -310;    //登录态验证失败
    const RET_AGENT_NOTIPHONE = -311;    //请求不是来至iPhone设备，请使用iPhone设备访问
	...
	static public $ARR_MSG 
		= array(  self::RET_SUCC => "成功"
				...
				, self::RET_T_TOPIC_ERR => "消息不能多于两条话题"
			    ...
			    , self::RET_API_KEYMISS => "appkey或者appsecret丢失"
			    , self::RET_MOBILE_CHECKSIGNATUREFAIL => "验证签名错误"
			    , self::RET_LOCAL_SAVEERROR => "消息存储失败"
			    , self::RET_MOBILE_USERINFOERROR => "获取用户信息失败"
			    , self::RET_MOBILE_DELFRIENDERROR => "取消收听失败"
			    , self::RET_MOBILE_ADDFRIENDERROR => "收听失败"
			    , self::RET_MOBILE_CHECKFRIENDERROR => "检测本地收听关系失败"
			    , self::RET_MOBILE_SYSTEMERROR => "系统错误"
			    , self::RET_NET_CONNECTERROR => "服务器网络通讯失败"
			    , self::RET_LOGINSTATE_INVALID => "登录态失效"
			    , self::RET_LOGINSTATE_FAIL => "登录态验证失败"
			    , self::RET_AGENT_NOTIPHONE => "请使用iPhone设备访问"
				);
