...
class Core_Controller_Action
{
    ...
    protected function _setDefaultTplParams ()
    {
		...
		Core_Template::assignvar(array(
			...
			'_gdurl' => 'index.php'.Core_Fun::getPathinfoPre().'gd',
			'_resource' => Core_Config::get('resource_path','basic','/'),
			'_pathroot' => Core_Fun::getPathroot(),	
			'_footCountMes' => 
				array(
					'appKey' =>  Core_Config::get('appkey', 'basic') ,		
					'accessToken' =>  empty($_SESSION['access_token'])?NULL:$_SESSION['access_token'] ,
					'version' =>  !defined('IWEIBO_VERSION')?'':IWEIBO_VERSION,
					'ip' =>  Core_Fun::ip(),
