...
class Model_Topic extends Core_Model
{
     ...
     * @return bool
     */
    public static function addTopic ($topic, $updateMblog = true,  $blog = null)
    {
        $obj = new self();
        if (is_array ($topic)) {
            ...
            if (!$_topic) {
                ...
                if (isset($blog['opentid'])&&strlen ($blog['opentid']) > 0
                        && $_topic['wall']
                    ...
                    self::addTopicBlogRelation ($_topic['tid'], $blog['opentid'], $visible);
                }
                $r = $obj->update ($_topic);
            }
        }
