...
class Model_Tiview extends Core_Model{
	...
	function saveTIview($tlive=array(),$style=array(),$users=array()){
		...
		$error = self::getUser($users[0],$this->_prefix);	
		!empty($error) && $ret['error'] = $error;
				
		$users[1] = array_unique(explode(";",preg_replace('/\s+/i',';',$users[1])));
		
		$same = array_filter(array_uintersect($users[0], $users[1], "strcasecmp"));
		...
		$error = self::getUser($users[1],$this->_prefix);	
		!empty($error) && $ret['error'] = $error;
