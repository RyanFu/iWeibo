...
if ($method == 'show_license') {//note 显示协议
    ...
    if ($submit) {//note 处理提交上来的数据
        ...
        $db->query ("INSERT INTO {$tablepre}mb_skin VALUES (NULL, '蒲公英', 'dandelion', '" . $setting_basic['resource_path'] . "view/dandelion/images/thumb.jpg', 2, 1)");
        //初始化其它数据
    ...
    if (VIEW_OFF) {
        ...
        //安装统计
        echo('<script>var u=["'.get_onlineip().'","","weibo.open.iweibo","install",0,565,"",encodeURIComponent(location.href),"iweibo'.SOFT_VERSION.'",0,"'.$_COOKIE['app_key'].'"];var s=new Image(1,1); s.src="http://btrace.qq.com/collect?sIp="+u[0]+"&iQQ="+u[1]+"&sBiz="+u[2]+"&sOp="+u[3]+"&iSta="+u[4]+"&iTy="+u[5]+"&iFlow="+u[6]+"&sUrl="+u[7]+"&sVersion="+u[8]+"&iReserve="+u[9]+"&sReserve="+u[10];</script>');
        //登录完跳转
