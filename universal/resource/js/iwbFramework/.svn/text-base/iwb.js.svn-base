...
(function () {
	...
	createNewPlayer = function () {
		if (window.ActiveXObject) {
		    playerWrapper.innerHTML = "<object id=\"mplayer\" classid=\"CLSID:6BF52A52-394A-11d3-B153-00C04F79FAA6\"><!--"+new Date().getTime()+"--></object>";
		} else {
    ...
    if (player.controls) { // mediaPlayer可用
        ...
        IWB_MUSIC_PLAYER = {
            ...
            src: function (url) { // 初始化音乐播放器对象
            	player=createNewPlayer();//破IE浏览器使用的mediaplayer不是只换个URL就能换首音乐
                this._src = url;
            },
...
IWB_VALIDATOR = {
    ...
    _regexp: {
                ...
                ,uname : [/^[^<>|\"]{3,15}$/i,"[label|帐号]只能含3-15个字符，不能包含敏感字符"] //由3-15个字符组成，不能包含<|"
...
(function () { //根据正则表达式自动添加方法
    ...
    for ( validator in IWB_VALIDATOR._regexp) {
        if ( IWB_VALIDATOR._regexp.hasOwnProperty(validator) ) {
        	...
        	IWB_VALIDATOR[validator] = new Function("str","return IWB_VALIDATOR._regexp[\"" + validator +"\"][0].test(str.replace(/[^x00-xff]/g,'aa')) ? \"\": IWB_VALIDATOR._regexp[\"" + validator + "\"][1]");	
        	}
        }
    }
}());
...
jQuery.fn.blink = function(maxblink,arg1,arg2){
    ...
    var times = 0; //写死为0，不再支持闪烁
    	maxblink = 0 ; //写死为0，不再需要闪烁
    var that = $(this);
    var replacement;
    var callback;
    var cTimes = 0;
...
$(function () {
    ...
    var createFriendSelector = function () { // 初始化选择器
       ...
       var searchFriends = function (kw) { // 格式化
           ...
           $.each(allFriend,function (index,friend) {
               if (!kw) { // 没有传入搜索key返回全部
                  ...
                  searchStartPos = friend.toLocaleLowerCase().search(kw.toLocaleLowerCase());
                  if(searchStartPos !== -1) { // 在搜索结果
                      ...
                      if (limitCharFriend !== friend) { // 处理截取后的字符高亮问题
                          if (searchStartPos+kw.length < limitCharFriend.length - 3 /*suff length*/) {
                              limitCharFriend = limitCharFriend.replace(new RegExp(kw,"gi"),function($1){return "<b>"+$1+"</b>";});
                          } else { //
           ...
           $.each(result, function (index,data) {
               resultHtml += ("<li data-username=\"" + resultUsername[index] + "\"><a href=\"javascript:;\">" + data + "</a></li>");
           });
           return resultHtml;
       };
           ...
           if (identity === "friendlist") {
               if(response && response.ret === 0) {
                   ...
                   allResult.find("li").mouseover(function () {
                   	   allResult.find("li").removeClass("active");
                       ...
                       currentTextarea.trigger("focus");
                       lastCaret = currentTextarea.get(0).caret; // 失去焦点时光标的位置
                       lastCaret = lastCaret || 0;
                       IWB_UTIL.insertText([currentTextarea.val().substr(lastCaret-1,1) =="@"?"":"@" ,name ," "].join("") ,lastCaret ,currentTextarea);
                       currentTextarea.get(0).caret += ["@" ,name ," "].join("").length; //更新光标位置
                       currentTextarea.trigger("keyup");
                   }).keydown(function(event) {
				var that = $(this).parent().find("li.active"),it;
				if(event.keyCode==40){
					it=that.nextAll().first();
					if(it.size()>0) {
						that.removeClass("active");
						it.addClass("active");
						if(it[0].offsetTop>=it[0].parentNode.clientHeight){
						it[0].parentNode.scrollTop = it[0].offsetTop;
						}
					}
					return false;
				}else if(event.keyCode==38){
					it=that.prevAll().first();
					if (it.size()>0) {
						that.removeClass("active");
						it.addClass("active");
						it[0].parentNode.scrollTop = it[0].offsetTop;
       ...
       searchInput.keyup(function (event) {
		   if(event.keyCode==13||event.keyCode==38||event.keyCode==40) { //屏蔽上下键、Enter键
		   return false;
		   }
           ...
           if (searchKw) {
			...
			if(event.keyCode==40||event.keyCode==38||event.keyCode==13){
				var that=((allResult.is(":visible")&&allResult)||(searchResult.is(":visible")&&searchResult)).find("li.active"),it;
				if(event.keyCode==40) {
					it=that.nextAll().first();
					if (it.size()>0) {
						that.removeClass("active");
						it.addClass("active");
						if(it[0].offsetTop>=it[0].parentNode.clientHeight){
							it[0].parentNode.scrollTop += it[0].offsetHeight;
						}
					}
				} else if (event.keyCode==38) {
					it = that.prevAll().first();
					if (it.size()>0) {
						that.removeClass("active");
						it.addClass("active");
						if (it[0].offsetTop<it[0].parentNode.scrollTop){
							it[0].parentNode.scrollTop -= it[0].offsetHeight;
						}
					}
    ...
    $(".iwbFriendControlBtn").live({
        click: function () {
            $(".iwbAutoCloseLayer").hide();
            var friendBtn = $(this);
            var friendSelector = $(".iwbFriendSelector"),friendSearch=friendSelector.find(".friendSearch").show(),friendDisplay=friendSelector.find(".friendDisplay").show();
    ...
    $(".iwbFriendControlInput").live({
        keyup : function (event) {
        	if (event.keyCode==38||event.keyCode==40||event.keyCode==13){return false;}
            var that = $(this);
            var val = that.val();
            ...
            if (kw) {
                ...
                if (!kw) { //无输入搜索
                    ...
                    friendSelector.find(".displayAll").show();
		...
		keydown:function(event){
		var friendSelector = $(".iwbFriendSelector"),friendDisplay=$(".displayResult"),displayAll=friendSelector.find(".displayAll");
		//console.log(friendDisplay.is(":visible")||displayAll.is(":visible"));
		// 如果选择器已存在并且是输入@触发的选择器
		if (friendSelector.is(":visible") && friendSelector.find(".close").is(":hidden")) {
				friendDisplay = (friendDisplay.is(":visible") && friendDisplay) || (displayAll.is(":visible") && displayAll);
            	if(event.keyCode==38||event.keyCode==40||event.keyCode==13){
            		var that=friendDisplay.find("li.active"),it;
            		if (event.keyCode==38) {
            			it=that.prevAll().first();
            			if (it.size()>0) {
            				that.removeClass("active");
...
IWB_SILDEWARE.prototype = {
    ...
    ,init: function () {
        ...
        sliderware.find(".close").click(function () {
            sliderware.remove();
            IWB_SESSION.set("showSlideAdv","true");
        });
        return sliderware;
    }
};
...
$(function(){
	...
	var _scrollWidth=$("#searchword").find("div")[0].scrollWidth;
	$("#searchword").mouseover(function(){
		$(this).find(".sleft,.sright").show();
	}).mouseout(function(){
		$(this).find(".sleft,.sright").hide();
	});
	$("#searchword").find(".sleft").click(function(){
		var t=$("#searchword").find("div");/*
		console.log([-parseInt(t.css("text-indent")),_scrollWidth,parseInt(t.css("text-indent"))+_scrollWidth].join(" "));
			if(parseInt(t.css("text-indent"))+_scrollWidth>0)
			{t.animate({"text-indent":parseInt(t.css("text-indent"))-t.width()},"fast");}
			else
			{return;}*/
			if(t.find("a:visible").size()>1)
			t.find("a:visible").first().animate({"width":0},100,function(){$(this).hide();});
	});
	$("#searchword").find(".sright").click(function(){
		var t=$("#searchword").find("div");/*
			if(parseInt(t.css("text-indent"))<0)
			{t.animate({"text-indent":parseInt(t.css("text-indent"))+t.width()},"fast");}
			else
			{return;}*/
			if(t.find("a:hidden").size()>0)
			{t.find("a:hidden").last().width("auto").fadeIn();}
	});
	}
	//兼容input和textarea的placeholder
	if (!!document.all) {
	$("input[placeholder][type='text'],textarea[placeholder]").blur(function(){
			var that=$(this),val=$(this).attr("placeholder");
			if (that.attr("unshowplaceholder")){
				return;	
			}
			if (that.val()==""){
					that.val(val).addClass("gray");
				}
		}).focus(function(){
			var that=$(this),val=$(this).attr("placeholder");
				if(that.val()==val){
...
var showAlertTip = function(str,cssobj,tarobj,timer) {
	...
	this.tip = function (cssobj) {
		var	o = $("<div class=\"errortip\"><b class=\"c\">◆</b><b class=\"b\">◆</b><div class=\"mes\"><!--{$showmsg}--></div>").appendTo($("body"));
		return o.css(cssobj);
	}(cssobj);
	this.reset =function (tarobj) {
		...
		tip.css({"top":dh+rect.top+tarobj.height()+10,"left":rect.left});
...
String.prototype.htmlencode = function(){
	...
	var CacheTopic ={ //初使化最近涉及的话题列表
		init:function(currentTarget){
		this.viewer=$("<ul class=\"iwbSessionTopicSelector\"></ul>").appendTo($("body"));
		this.target = $(currentTarget);
		this.currentTarget=currentTarget;
		this.initEvent();
		},
		reset:function(regstr) {
			this.viewer.find("li").unbind("keydown").unbind("click");
			topicstr=IWB_LOCALSTORAGE.get("sessionTopics")||"%E8%BE%93%E5%85%A5%E8%AF%9D%E9%A2%98%E6%A0%87%E9%A2%98";
			if (regstr){
				var toparr=topicstr.split("\n");
				for (var i=0;i<toparr.length;i++)
				{	
					if(!(new RegExp(regstr.replace(/[\(\)\-\+\?\[\]\{\}\\\/\$\^\.\*\|]/g,"\\$1"),"ig").test(decodeURIComponent(toparr[i])))){
					delete toparr[i];
					}
				}
				topicstr=toparr.join("|").replace(/\|+/g,"\n");
				this.regstr="#"+regstr;
			}
			var topiclist=topicstr.replace(/([^\n]+)/g,function($1){var t=decodeURIComponent($1).htmlencode();return "<li title=\""+t+"\"><a href=\"javascript:;\">"+t+"</a></li>";});
			this.viewer.html(topiclist).find("li").first().addClass("active");
			if(this.viewer.find("li").size()>0){
			this.bindEvent();
			}else{
			this.viewer.hide();
			}
		...
		initEvent:function(){
			var _this=this;
			$(_this.currentTarget).live({
				"keyup":function(event){
					if(event.keyCode==38||event.keyCode==40||event.keyCode==13||event.keyCode==undefined){return false;}
					var that=$(this);
					var val= that.val();
					var caret= that.caret()||0;
					var kw= val.substring(0,caret).match(/#([^#]*)$/);
						if(kw){
							kw=kw[1];
							_this.textinsert={
							"caret":caret-kw.length,
							"pretext":val.substring(0,caret-kw.length),
							"aftertext":val.substring(caret)
							};
							if (!CacheTopic.viewer.is(":visible")){
								var caretOffset=that.caretOffset(),thatOffset = that.offset();
								selectorLeft = thatOffset.left + caretOffset.left;
								selectorTop = thatOffset.top + Math.min(that.height(),caretOffset.top);
								CacheTopic.viewer.css({
								"left":thatOffset.left + caretOffset.left,
								"top":thatOffset.top + Math.min(that.height(),caretOffset.top)+5
								}).show();
							}
							_this.target=that;
							CacheTopic.reset(kw);
				...
				"keydown":function(event){
					...
					if (event.keyCode==38||event.keyCode==40){
						that=_this.viewer.find("li.active");
						it=(event.keyCode==38?that.prevAll().first():that.nextAll().first());
						if(it.size()>0){
							that.removeClass("active");
							it.addClass("active");
						}
						return false;
					}else if(event.keyCode==13&&_this.viewer.is(":visible")){
						_this.viewer.find("li.active").trigger("click");
						return false;
					}
				},
				"blur":function(){
					var that=$(this),val=that.val(),sessiontopic,sessionarr=[];
					thattopic=val.match(/#([^#\s]+)?#/g);
					if(IWB_LOCALSTORAGE){
					sessiontopic=IWB_LOCALSTORAGE.get("sessionTopics");
						if(sessiontopic){
						sessionarr=decodeURIComponent(sessiontopic).split("\n");
							for(var i=0;i<=sessionarr.legnth-1;i++){
							sessionarr[i]=decodeURIComponent(sessionarr[i]);
							}
						}
						if(thattopic&&thattopic.length>0){
							for(var i=0;i<thattopic.length;i++){
							var topicname=thattopic[i].replace(/#/g,"");
							if(topicname.length<20)
								{sessionarr.push(topicname);}
							}
						}
						sessionarr=sessionarr.unique();
						for (var i=0;i<sessionarr.length;i++){
						sessionarr[i]=encodeURIComponent(sessionarr[i]);
						}
						if(sessionarr.length>6)
						sessionarr=sessionarr.slice(sessionarr.length-6,sessionarr.length);
						IWB_LOCALSTORAGE.set("sessionTopics",sessionarr.join("\n"));
					}
					setTimeout(function(){_this.viewer.hide();},100);
				}
				});
		},
		bindEvent:function() {
			var _this=this;
			_this.viewer.find("li").bind({
			"click":function(){
				_this.target.val([_this.textinsert.pretext,_this.textinsert.aftertext].join(""));
				IWB_UTIL.insertText($(this).find("a").text()+"#" ,_this.textinsert.caret ,_this.target);
				CacheTopic.viewer.hide();
				_this.target.trigger("keyup");
			},
			"mouseover":function(){
	...
	CacheTopic.init(".iwbTopicControlInput");