...
$(function () {
    ...
    $("#msgTxt").keyup(function(event) {
    	...
    	if(IWB_LOCALSTORAGE){
    		IWB_LOCALSTORAGE.set("sessionTopicMsgTxt",$("#msgTxt").val(),30);
    	}
    });;
    ...
    if ($("#msgTxt").attr("data-default")) {
        $("#msgTxt").focus();
        var curtopic=$("#msgTxt").attr("data-default");
        if(IWB_LOCALSTORAGE.get("sessionTopicMsgTxt")!=null) {
        	var sessionMsg=IWB_LOCALSTORAGE.get("sessionTopicMsgTxt");// 重置表单
        	if (sessionMsg.indexOf(curtopic)!=-1) {
        		$("#msgTxt").val(sessionMsg);
        	} else {
        		$("#msgTxt").val(curtopic);
        	}
        } else {
