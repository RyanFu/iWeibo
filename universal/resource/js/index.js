           ...
           ,getDOM: function () {
               ...
               IWB_API.listSkin("listskin" ,function (identity ,response) {
                   ...
                   if (response.ret === 0) {
                        ...
                        IWB_DIALOG.resizeTo(boxId, 590, 98+102*Math.ceil(skins.length/4));
                   } else {
...
$(function () {
    // 检查字数
    if ($("#msgTxt").size()>0 && IWB_LOCALSTORAGE.get("sessionMsgTxt")!=null && IWB_LOCALSTORAGE.get("sessionMsgTxt")!=""){
    	$("#msgTxt").val(IWB_LOCALSTORAGE.get("sessionMsgTxt"));
    }
    ...
    $("#msgTxt").keyup(function(event) {
    	...
    	if(IWB_LOCALSTORAGE){
    		IWB_LOCALSTORAGE.set("sessionMsgTxt",$("#msgTxt").val(),30);
    	}
    });
        ...
        $("#shipintxt").get(0).onResponse = function (identity,response) {
            if(identity === this.getAttribute("data-identity")){
                if (response.ret === 0) { // 视频有效
                    ...
                    response.data.title=response.data.title.replace(/&#\d+;/g,"");
                    sendVideo.hide();
                    videoPreviewControl.attr("data-imageUrl",response.data.minipic);
                    videoPreviewControl.text(IWB_UTIL.limit(response.data.title.replace(/&#\d+;/g,""),6));
                    videoPreviewer.show();
                    IWB_UTIL.highlightOrInsert("分享视频",$("#msgTxt").get(0),"#",response.data.title+" "+response.data.real);
                    $("#msgTxt").trigger("keyup");
                    ...
                    videoPreviewer.find(".del").unbind('click').bind('click',function () { // 取消添加视频
                        ...
                        $("#msgTxt").val($("#msgTxt").val().replace(new RegExp("\\s*#分享视频#\\s*","g"),"")).trigger("keyup");
                        sendVideo.show();
                    });
                } else {
                        ...
                        if (response.msg === "error") {
                        	...
                        	$(".iwbAddVideo").find(".videoInfo").html("获取视频信息失败："+response.msg).show();
                        	$(".iwbAddVideo").find("button[class=videoSubmit]").prop("disabled",false);
                        }
                }
            }
        ...
        $("#yinyuetxt").get(0).onResponse = function (identity,response) {
            if(identity === this.getAttribute("data-identity")){
                if (response.ret === 0) {
                    ...
                    IWB_UTIL.highlightOrInsert("分享音乐",$("#msgTxt").get(0),"#",response.data.url);
                    $("#msgTxt").trigger("keyup");
                    ...
                    $("#yinyuetxt").parent().find(".del").click(function () { // 删除音乐
                        ...
                        $(this).remove();
                        $("#msgTxt").val($("#msgTxt").val().replace(new RegExp("\\s*#分享音乐#\\s*","g"),"")).trigger("keyup");
                    });
                } else {
    ...
    $("#newTopic").click(function () {
    	$("#msgTxt").trigger("focus");
    	var caret = ($("#msgTxt").get(0).caret !== undefined ? $("#msgTxt").get(0).caret : $("#msgTxt").val().length) ;
    	$("#msgTxt").get(0).caret=caret;
    ...
    var validateForm = function () {
        ...
        if (text.replace(/#.*#/g,"").replace(/\s+/g,"").length==0) { //微博内容不能只含话题
        	errormsg = "再说点什么吧";	
        }
    ...
    var uploadPicHandler = function () {
        ...
        cancelPic.click(function () {
            ...
            $("#msgTxt").val($("#msgTxt").val().replace(new RegExp("(\\s*#分享照片#\\s*)","g"),"")).trigger("keyup");;
        });
...
window.onSendboxResponse = function (response) {
    ...
    if (response.ret===0){
        ...
        if (IWB_LOCALSTORAGE) {
		IWB_LOCALSTORAGE.del("sessionMsgTxt");
		}
...
$(function () {
    ...
    $(".timelinefilter").change(function(){
        ...
        var urlbase = location.href.match(/(.*)(\/u|\?u)\/\w+(\/[cu]type\/\d)?/);
    ...
    var replyForm = function () {
        ...
        instance.find(".icon_topic").click(function() {
        	var caret = (forminput.get(0).caret !== undefined ? forminput.get(0).caret : forminput.val().length) ;
        	forminput.get(0).caret = caret;
...
$(function () {
    var createGuestCard = function (guest) {
        ...
        var cardinfo = ["<div class=\"ucard\">"
                       ...
                       ,    "<a href=\"javascript:void(0);\" data-name=\"" + guest.name + "\" data-styleid=\"1\" class=\"iwbFollowControl follow" + (guest.ismyidol === 1 ? " unfollow":"") + "\" data-type=\"" + (guest.ismyidol === 1 ? "0":"1") + "\" title=\"" + (guest.ismyidol === 1 ? "取消":"") + "收听\"></a>"
                       ,    "<div class=\"uinfo\">"
                       ...
                       ,            "<li class=\"privatemail\"> <a href=\"javascript:void(0);\" class=\"" + (guest.ismyfans === 1 ? ("sendmail\" data-receiver=\"" + guest.name + "\"") : "gray\"") + ">私信</a> </li>"
...
$(function () {
    ...
    updateFootControlPos = function (total , scrolled, visible) {
        ...
        if (!ie6) {
            ...
            footControls.css({
               "bottom": Math.max(0 ,visible + scrolled - (total - footerHeight /*- 4*/)-parseInt($(".footer").css("padding-top")))
            });
        } else {
        ...
        IWB_API.updateNotice("updatenotice" ,function (identity ,response) {
            ...
            if (identity === "updatenotice" && response && response.ret === 0) {
                ...
                if (data.fans > 0) { // 听众更新数
                	if (data.fans <100) {
                		data.fans = "+"+data.fans;
                	}else if(data.fans>= 1000){
                		data.fans = "999+";	
                	}
                    $("#newfans").show().find("em").text(data.fans);
           ...
           setTimeout(function(){updateNotice();},30*1000); 
        });
    };
        ...
        boxId = IWB_DIALOG._init({
           ...
           ,getDOM: function () {
               var talkBox =  ["<div class=\"replyform\" style=\"border:none;text-align:left;\">"
                                  ...
                                  ,"    <ul class=\"top\">"
                                  ,"        <li>"
                                  ,"            <textarea name=\"talkboxinput\" id=\"iwbtalkinput\" class=\"iwbTopicControlInput\"></textarea>"
                                  ,"        </li>"
                                  ,"    </ul>"
                                  ,"        <div class=\"fleft\">"
