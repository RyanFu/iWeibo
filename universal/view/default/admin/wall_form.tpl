...
<form name="cpform" method="post" action="/admin/wall/<!--{$_actionName}-->" id="cpform" onsubmit="return $.checkForm(this)">
    ...
    <table class="tb tb2 ">
        ...
        <tr class="noborder">
            <td class="vtop rowform">
                ...
                <!--{if 'add' == $_actionName}--><input name="title" name="title" value="<!--{$wall.title}-->" type="text" class="txt" datatype="Require" msg="请填写话题名称"/><!--{/if}-->
            </td>
            <td class="vtop tips2"><span info="title"></span><!--{if 'add' == $_actionName}-->添加以后不可修改<!--{/if}--></td>
        </tr>
        ...
        <tr class="noborder">
            <td>
                <input type="text" class="txt" name="wallstarttime_Date" value="<!--{$wall.wallstarttime|date_format:"%Y-%m-%d"}-->" onclick="showcalendar(event, this)" datatype="Date" msg="请填写正确的开始时间">
                 <!--{html_select_time use_24_hours=true time=$wall.wallstarttime prefix='wallstarttime_'}-->
            </td><td class="vtop tips2"><span info="wallstarttime_Date"></span></td></tr>
        <tr><td colspan="2" class="td27">结束时间</td></tr>
        <tr class="noborder">
            <td>
                 <input type="text" class="txt" name="wallendtime_Date" value="<!--{$wall.wallendtime|date_format:"%Y-%m-%d"}-->" onclick="showcalendar(event, this)" datatype="Date" msg="请填写正确的结束时间">
                 <!--{html_select_time use_24_hours=true time=$wall.wallendtime prefix='wallendtime_'}-->
            </td><td class="vtop tips2"><span info="wallendtime_Date"></span></td></tr>
