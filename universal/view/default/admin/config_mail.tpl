...
<form action="/admin/config/mail" method="post" onsubmit="var f=$(this),opened=f.find('input[name=\'config[mail][open]\']:checked').val(),opentype=f.find('input[name=\'config[mail][type]\']:checked').val();return $.checkForm($('#mail_'+opentype))">
    <table class="tb tb2">
        <tr>
        ...
        <tbody id="mail_smtp" <!--{if $mail_type != 'smtp' || $mail_on == 0}-->style="display: none;"<!--{/if}-->>
        	<tr>
        		<td colspan="2"><a href="http://open.t.qq.com/bbs/forum.php?mod=viewthread&tid=9617&extra=page%3D4" target="_blank">点击此处查看如何设置？</a></td>
        	</tr>
            <tr>
            ...
            <tr class="noborder">
                <td class="vtop rowform">
                    <input type="text" class="txt" name="config[mail][smtp_server]" value="<!--{TO->cfg key="smtp_server" group="mail" default=""}-->" datatype="Require" msg="请填写SMTP 服务器地址"/>
                </td>
                <td class="vtop tips2"><span info="config[mail][smtp_server]"></span>推荐使用 QQ 企业邮箱：<a href="http://exmail.qq.com" target="_blank">http://exmail.qq.com</a></td>
            </tr>
            <tr>
            ...
            <tr class="noborder">
                <td class="vtop rowform">
                    <input type="text" class="txt" name="config[mail][smtp_port]" value="<!--{TO->cfg key="smtp_port" group="mail" default=""}-->" datatype="Number" msg="请填写正确的端口号"/>
                </td>
            ...
            <tr class="noborder">
                <td class="vtop rowform">
                    <input type="text" class="txt" name="config[mail][sender]" value="<!--{TO->cfg key="sender" group="mail" default=""}-->" datatype="Email" msg="请填写正确的邮箱地址"/>
                </td>
