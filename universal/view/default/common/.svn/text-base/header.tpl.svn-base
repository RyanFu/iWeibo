...
<div class="wrapper navbar">
    ...
    <div class="searchbar">
    	<form name="searchForm" class="search" method="post" action="/search/<!--{if isset($headsearch)}--><!--{$headsearch}--><!--{else}-->all<!--{/if}-->">
        ...
        <div class="hotsearch">
        	<strong>关键字</strong>
	        <div class="searchword" id="searchword"><div>
	         <!--{foreach key=key item=word from=$hotWords}-->
	            <a href="/search/all/k/<!--{$word|iurlencode}-->"><!--{$word}--></a>
	         <!--{/foreach}-->
	        </div>
	         <a href="javascript:;" class="sleft"></a>
	         <a href="javascript:;" class="sright"></a>
	         </div>
        </div>
        -->
    </div>
